...
def get_parameter_names(dpp = True):
    p = ["ln_likelihood"]
    if dpp:
    ...
    for l in comparison_labels:
        p.append("ln_likelihood_{0}".format(l))
        p.append("root_height_{0}".format(l))
        p.append("mutation_rate_{0}".format(l))
        p.append("freq_1_{0}".format(l))
        p.append("pop_size_{0}".format(l))
        p.append("pop_size_root_{0}".format(l))
    return p

def get_results_header(number_of_comparisons = 6, dpp = True):
    h = [
            "batch",
            "sim",
            "sample_size",
    ...
    for p in get_parameter_names(dpp = dpp):
...
def parse_simulation_results(
    ...
    val_sim_dirs = glob.glob(os.path.join(project_util.ECOEVOLITY_SIM_DIR, '*'))
