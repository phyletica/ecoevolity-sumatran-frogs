...
def main_cli(argv = sys.argv):
    ...
    full_results = parse_results(glob.glob(
            os.path.join(project_util.ECOEVOLITY_SIM_DIR,
                    ...
                    "batch*",
                    "results.csv.gz")))
    exonic_results = parse_results(glob.glob(
            os.path.join(project_util.ECOEVOLITY_SIM_DIR,
                    "exonic_sim",
                    "batch*",
                    "results.csv.gz")))
    full_doubled_results = parse_results(glob.glob(
            os.path.join(project_util.ECOEVOLITY_SIM_DIR,
                    "full_doubled_sim",
                    "batch*",
                    "results.csv.gz")))
    ...
    column_labels = ["Full", "Exonic"]
    doubled_column_labels = ["Full", "Doubled full"]

    height_parameters = []
    root_size_parameters = []
    leaf_size_parameters = []
    height_ess_parameters = []
    height_psrf_parameters = []
    for l in comparison_labels:
        height_parameters.append("root_height_{0}".format(l))
        root_size_parameters.append("pop_size_root_{0}".format(l))
        leaf_size_parameters.append("pop_size_{0}".format(l))
        height_ess_parameters.append("ess_sum_root_height_{0}".format(l))
        height_psrf_parameters.append("psrf_root_height_{0}".format(l))
    ...
    for parameter, p_info in parameters_to_plot.items():
        full_data = ScatterData.init(full_results,
                p_info["headers"],
                highlight_parameter_prefix = "psrf",
                highlight_threshold = brooks_gelman_1998_recommended_psrf)
        exonic_data = ScatterData.init(exonic_results,
                p_info["headers"],
                highlight_parameter_prefix = "psrf",
                highlight_threshold = brooks_gelman_1998_recommended_psrf)
        full_doubled_data = ScatterData.init(full_doubled_results,
                p_info["headers"],
                highlight_parameter_prefix = "psrf",
                highlight_threshold = brooks_gelman_1998_recommended_psrf)
        ...
        data_grid = [
                [full_data, full_doubled_data],
                ]

        generate_scatter_plots(
                data_grid = data_grid,
                plot_file_prefix = "full-doubled-" + parameter,
                parameter_symbol = p_info["symbol"],
    ...
    full_smallest_data = ScatterData.init(full_results,
            ["root_height_poes"],
            highlight_parameter_prefix = "psrf",
            highlight_threshold = brooks_gelman_1998_recommended_psrf)
    full_largest_data = ScatterData.init(full_results,
            ["root_height_cats"],
            highlight_parameter_prefix = "psrf",
            highlight_threshold = brooks_gelman_1998_recommended_psrf)
    ...
    data_grid = [
            [full_smallest_data, full_largest_data],
            ]
    ...
    generate_scatter_plots(
            data_grid = data_grid,
            plot_file_prefix = "full-event-time-sampling-disparity",
            parameter_symbol = "t",
    ...
    results_grid = [
            [full_results, full_doubled_results],
            ]
    generate_model_plots(
            results_grid = results_grid,
            column_labels = doubled_column_labels,
    ...
    full_data = HistogramData.init(full_results, parameters, True)
    exonic_data = HistogramData.init(exonic_results, parameters, True)
    full_doubled_data = HistogramData.init(full_doubled_results, parameters, True)
    ...
    generate_histograms(
            data_grid = data_grid,
            plot_file_prefix = "full-doubled-number-of-variable-sites",
            column_labels = doubled_column_labels,
            row_labels = None,
            parameter_label = "Number of variable sites",
            range_key = "range",
    ...
    histograms_to_plot = {
            ...
            "psrf-event-time": {
                    "headers": height_psrf_parameters,
    ...
    for parameter, p_info in histograms_to_plot.items():
        full_data = HistogramData.init(full_results, p_info["headers"], False)
        exonic_data = HistogramData.init(exonic_results, p_info["headers"], False)
        full_doubled_data = HistogramData.init(full_doubled_results, p_info["headers"], False)
